			...
			if recv_msg != None:
				if resid == recv_msg.arbitration_id:
					if recv_msg.data[0] == 0x00 :
						...
						target = float(int.from_bytes(recv_msg.data[1:5], byteorder='little', signed=True))/0x10000
						input = float(int.from_bytes(recv_msg.data[5:9], byteorder='little', signed=True))/0x10000
						output = float(int.from_bytes(recv_msg.data[9:13], byteorder='little', signed=True))/0x10000
